...
import static shipwrights.genesis.client.ShaderRegistry.getPlanetCloudsRenderType;

public class PlanetCloudsRenderer implements CelestialRenderer {
    ...
    public void invoke(@NotNull RenderLevelStageEvent event, @NotNull Celestial toRender, @NotNull VantagePoint vantagePoint) {
        ...
        VertexConsumer cloudsBuffer = bufferSource.getBuffer(getPlanetCloudsRenderType());
        renderClouds(event.getPoseStack(), cloudsBuffer, size, position, rotation, event, toRender, vantagePoint);
        bufferSource.endBatch(getPlanetCloudsRenderType());
    }

    private void renderClouds(PoseStack poseStack, VertexConsumer buffer, double size, Vector3dc center, Quaterniondc localRotation, @NotNull RenderLevelStageEvent event, @NotNull Celestial toRender, VantagePoint vantagePoint) {
        ...
        float relativeCloudHeight = 1.0f + 0.05f * (float) props.atmosphere().thickness();
        ...
        ShaderInstance shader = ShaderRegistry.PLANET_CLOUDS_SHADER.getInstance().get();
        ...
        Uniform uniformThickness = shader.getUniform("CloudThickness");
        if (uniformThickness != null) {
            uniformThickness.set(relativeCloudHeight);
        }
        ...
        Uniform uniformDensityFade = shader.getUniform("DensityFade");
        if (uniformDensityFade != null) {
            uniformDensityFade.set((float) densityFade);
        }
        ...
        boolean cameraInsideAtmosphere = (vantagePoint instanceof VantagePoint.OnCelestial oc && oc.celestial().equals(toRender))
                || (Math.abs(testPos.x) < halfSize * relativeCloudHeight
                    && Math.abs(testPos.y) < halfSize * relativeCloudHeight
                    && Math.abs(testPos.z) < halfSize * relativeCloudHeight);

        addCubeFaceClouds(matrix, buffer, -halfSize, -halfSize, halfSize, halfSize, -halfSize, halfSize, halfSize, halfSize, halfSize, -halfSize, halfSize, halfSize,  halfSize, correctionDist, relativeCloudHeight,outOverexposureCancel);
        addCubeFaceClouds(matrix, buffer, -halfSize, -halfSize, -halfSize, -halfSize, halfSize, -halfSize, halfSize, halfSize, -halfSize, halfSize, -halfSize, -halfSize,  halfSize, correctionDist, relativeCloudHeight,outOverexposureCancel);
        addCubeFaceClouds(matrix, buffer, -halfSize, -halfSize, -halfSize, -halfSize, -halfSize, halfSize, -halfSize, halfSize, halfSize, -halfSize, halfSize, -halfSize,  halfSize, correctionDist, relativeCloudHeight,outOverexposureCancel);
        addCubeFaceClouds(matrix, buffer, halfSize, -halfSize, -halfSize, halfSize, halfSize, -halfSize, halfSize, halfSize, halfSize, halfSize, -halfSize, halfSize,  halfSize, correctionDist, relativeCloudHeight,outOverexposureCancel);
        addCubeFaceClouds(matrix, buffer, -halfSize, -halfSize, -halfSize, halfSize, -halfSize, -halfSize, halfSize, -halfSize, halfSize, -halfSize, -halfSize, halfSize,  halfSize, correctionDist, relativeCloudHeight,outOverexposureCancel);
    ...
    private static void addCubeFaceClouds(Matrix4f matrix, VertexConsumer buffer, float x1, float y1, float z1, float x2, float y2, float z2,
                                          float x3, float y3, float z3, float x4, float y4, float z4, float halfSize, float zFightingCorrection, float cloudHeight, float outFact) {

        float size = 2 * halfSize;
        float sclFct;

        sclFct = zFightingCorrection * cloudHeight;
        buffer.vertex(matrix, x1 * sclFct, y1 * sclFct, z1 * sclFct).color((int)(255 * (x1 + halfSize) / size), (int)(255 * (y1 + halfSize) / size), (int)(255 * (z1 + halfSize) / size), 255).endVertex();
        buffer.vertex(matrix, x2 * sclFct, y2 * sclFct, z2 * sclFct).color((int)(255 * (x2 + halfSize) / size), (int)(255 * (y2 + halfSize) / size), (int)(255 * (z2 + halfSize) / size), 255).endVertex();
        buffer.vertex(matrix, x3 * sclFct, y3 * sclFct, z3 * sclFct).color((int)(255 * (x3 + halfSize) / size), (int)(255 * (y3 + halfSize) / size), (int)(255 * (z3 + halfSize) / size), 255).endVertex();
        buffer.vertex(matrix, x4 * sclFct, y4 * sclFct, z4 * sclFct).color((int)(255 * (x4 + halfSize) / size), (int)(255 * (y4 + halfSize) / size), (int)(255 * (z4 + halfSize) / size), 255).endVertex();
    }
}